...
def _resolved_stem(stem: str) -> str:
    """Return stem for ImageNet models: validate it or fall back to imagenet_standard."""
    return stem if stem in ("imagenet_standard", "imagenet_deep") else "imagenet_standard"


def build_model(cfg: Config) -> torch.nn.Module:
    """Build the model matching cfg.model.arch.

    Raises ValueError on an unknown arch. ImageNet models receive the resolved
    stem plus SE/attention/drop-path/downsample extras; CIFAR models share
    num_classes/shortcut/width/attention/se/droppath.
    """
    m = cfg.model
    extras = dict(
